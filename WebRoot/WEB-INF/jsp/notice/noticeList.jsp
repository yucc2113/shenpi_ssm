...
function ajaxSubmit(pageNum, searchName, startDate, endDate) {
		$.ajax({
			type : "post",
			url : '${pageContext.request.contextPath}/notice/ajaxNoticeList.action',
			data : {
				pageNum : pageNum,
...
function toAdd(){
	 document.forms.noticeFrom.action="${pageContext.request.contextPath}/notice/noticeAdd.action";
...
function toUpdate(){
	...
	if(number==1){	
			...
			document.forms.noticeFrom.action="${pageContext.request.contextPath}/notice/noticeUpdate.action?id="+id;
			document.forms.noticeFrom.submit();				
		}else{
...
function toDelete(){
	...
	if(number>0){
		 ymPrompt.confirmInfo({title:'信息确认',message:'确认要删除吗？',handler: function(tp){
			if(tp=='ok'){
				document.forms.noticeFrom.action="${pageContext.request.contextPath}/notice/noticeDelete.action";
				document.forms.noticeFrom.submit();
				return true;
			}else{
				return false;
...
function toLook(id){
	 document.forms.noticeFrom.action="${pageContext.request.contextPath}/notice/noticeDetail.action?id="+id;
	 document.forms.noticeFrom.submit();
}
</script>
</head>
