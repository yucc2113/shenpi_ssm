...
{
	$("#dealPosition").html(D.document.getElementById('dealPosition').value);
        	...
        	Ext.BLANK_IMAGE_URL = "<%=path %>/extjs/resources/images/default/s.gif";
		     ...
		     get_tree = new Ext.tree.TreePanel({
		        ...
		        loader: new Ext.tree.TreeLoader({
		        	dataUrl: 'flowstep!choosePosition.action',
		        	listeners: {
			            beforeload: function(treeLoader,node) {
			        		treeLoader.dataUrl = '${pageContext.request.contextPath}/position/getPositionTree.action';
			            }
			        },
...
function toValue(node,checked){
	...
	var names1 ="";
   	...
   	if(checked){
		 ...
		 $("#dealPosition").html(ids);
	     $("#resultName").html(names);
   	}else{
   		var dealPosition = $("#dealPosition").html().split(",");
   		var resultName = $("#resultName").html().split(",");
   		
   		for(i=0;i<dealPosition.length;i++){
				if(node.id!=dealPosition[i]){
					if(ids1==""){
					ids1 = dealPosition[i] ;
					}else{
					ids1 = ids1+","+dealPosition[i] ;
					}
				}
			}
...
function setValue(){
	    var id = $("#dealPosition").html();
	    var name = $("#resultName").html();
	    D.document.getElementById('dealPosition').value=id;
	    D.document.getElementById('libusername').value=name;
	    D.document.getElementById('stepType').value='3';
		P.cancel();
...
<input type="hidden" name="name" id="name" value="<%=request.getParameter("name")%>" />
	...
	<div id="dealPosition" style="display:none;"></div>
