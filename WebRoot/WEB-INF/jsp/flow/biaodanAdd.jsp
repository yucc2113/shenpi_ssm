...
$(document).ready(function() {
	...
	$.formValidator.initConfig({formid:"flowForm",onerror:function(msg){return false}});
});

function toSubmit(){
	
		document.forms.flowForm.action="${pageContext.request.contextPath}/flowdefine/biaodanAddSave.action";
...
<body>
<div id="right">
<form name="flowForm"  validate="true" theme="simple" id="flowForm" method="post">
<input type="hidden" name="id" id="id" value= "${id}" />
	<div class="right_box">
		<div class="menu_div"><ul class="menu_title">当前位置： 流程管理-选择表单</ul></div>
	</div>
		<div class="com_contant">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="data_list_b" summary="部门新增" id="intertable">
				...
				<tr>
					<th>表&nbsp;&nbsp;&nbsp;&nbsp;单</th>
			        <td>
			           <select  id="biaodanId" name="biaodanId" class="select_318" >
			               <option value="-1">请选择</option>
			              
						   <c:forEach var="biaodan" items="${biaodanList }">
									<c:if test="${flow.biaodanId ==biaodan.id}">
										<option value="${biaodan.id}" selected="selected">${biaodan.mingchen}</option>
									</c:if>
									<c:if test="${flow.biaodanId !=biaodan.id }">
						            <option value="${biaodan.id}">${biaodan.mingchen }</option>
						        </c:if>
